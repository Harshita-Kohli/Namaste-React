...
const Body = () => {
  ...
  const fetchData = async () => {
    ...
    // append a proxy url before the actual one to get rid of CORS error
    const data = await fetch("https://thingproxy.freeboard.io/fetch/" + RES_LIST_URL);
        ...
        <div>
          <button className="filter-btn mr-4 p-2 bg-slate-300 rounded-md hover:shadow-md"
            onClick={() => {  //this is a callback function that runs when we click the button
              ...
              //update the state:
              setFilteredRestaurants(filteredRestaurants);//this updates the state variable and leads to re-rendering of the component!
